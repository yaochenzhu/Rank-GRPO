...
class RankGRPOTrainer(Trainer):
    ...
    def _segment_items_from_tokens(
        self,
        completion_ids: "torch.Tensor",      # (B, T)
        completion_mask: "torch.Tensor",     # (B, T), 0/1, includes EOS
        max_items: int                       # rec_num
    ) -> tuple["torch.Tensor", "torch.Tensor", "torch.Tensor"]:
        """
        ### NEW/CHANGED
        Assign an *unbounded* item index to each token (0,1,2,...); newline tokens
        are attached to the *previous* item and increase the counter *after* them.
        Returns:
          item_seg_ids:        (B, T) long; natural item indices (may exceed max_items-1)
          item_content_mask:   (B, T) bool; tokens belonging to the first `max_items` items (incl. newlines)
          overflow_token_mask: (B, T) bool; tokens after the first `max_items` items (before EOS)
        """
        device = completion_ids.device
        B, T = completion_ids.size()

        item_seg_ids = torch.full((B, T), -1, dtype=torch.long, device=device)

        # A tokenizer for per-token decode
        tok = getattr(self, "_tok_for_decode", None)
        if tok is None:
            try:
                from transformers import ProcessorMixin, PreTrainedTokenizerBase  # type: ignore
                tok = self.processing_class.tokenizer if isinstance(self.processing_class, ProcessorMixin) else self.processing_class
            except Exception:
        ...
        for b in range(B):
            valid = int(completion_mask[b].sum().item())
            if valid <= 0:
                continue
            row = completion_ids[b, :valid]

            sep_counts = [0] * valid
            if newline_ids and len(newline_ids) == 1:
                nl_id = int(newline_ids[0])
                for t in range(valid):
                    if int(row[t].item()) == nl_id:
                        sep_counts[t] = 1
            ...
            for t in range(valid):
                if sep_counts[t] == 0:
                    tid = int(row[t].item())
                    try:
                        piece = tok.decode([tid], clean_up_tokenization_spaces=False, skip_special_tokens=False)
                    except TypeError:
                    ...
                    p2 = piece.replace("\r\n", "\n")
                    if "\n" in p2:
                        sep_counts[t] = p2.count("\n")

            item_id = 0
            for t in range(valid):
                item_seg_ids[b, t] = item_id
                if sep_counts[t] > 0:
                    item_id += sep_counts[t]

        item_content_mask   = completion_mask.bool() & (item_seg_ids >= 0) & (item_seg_ids < max_items)
        overflow_token_mask = completion_mask.bool() & (item_seg_ids >= max_items)
        return item_seg_ids, item_content_mask, overflow_token_mask
    ...
    def __init__(
        ...
        # Keep a reference to a tokenizer for per-token decoding
        self._tok_for_decode = tokenizer
        ...
        # Fixed number of recommendations to expect; prefer args.rec_num when provided
        self.rec_num = rec_num
        
        # ### NEW/CHANGED: hyperparams for length shaping (safe defaults)
        self.end_of_list_reward = getattr(args, "end_of_list_reward", 0.1)   # + reward on EOS when exactly 20
        self.extra_token_penalty = getattr(args, "extra_token_penalty", -0.1) # - per-token penalty after 20
    ...
    def _calculate_rewards(self, inputs, prompts, completions, completion_ids_list):
        ...
        for reward_func, reward_processing_class, reward_func_name in zip(
            ...
            names.append(reward_func_name)
            with profiling_context(self, reward_func_name):
                if isinstance(reward_func, nn.Module):  # Reward as classifier → scalar per sequence
                    ...
                    with torch.inference_mode():
                        scores = reward_func(**reward_inputs).logits[:, 0].float()  # (N,)
                    per_func_outputs.append(scores.unsqueeze(1).to(device))  # (N,1)
                else:
                    out = reward_func(prompts=prompts, completions=completions, completion_ids=completion_ids_list, **reward_kwargs)
                    # Accept List[float] or List[List[float]]
                    if isinstance(out[0], (list, tuple)):
                        rows = []
                        max_len = max(len(x) if x is not None else 0 for x in out)
                        if max_len == 0:
                            max_len = 1
                        for x in out:
                            if x is None:
                                rows.append(torch.full((max_len,), float("nan")))
                            else:
                                xt = torch.tensor(list(x), dtype=torch.float32)
                                if xt.numel() < max_len:
                                    xt = F.pad(xt, (0, max_len - xt.numel()), value=float("nan"))
                                rows.append(xt)
                        per_func_outputs.append(torch.stack(rows, dim=0).to(device))  # (N, rec_num)
                    else:
                        vals = [xi if xi is not None else float("nan") for xi in out]
                        per_func_outputs.append(torch.tensor(vals, dtype=torch.float32, device=device).unsqueeze(1))  # (N,1)
        ...
        rewards_per_func = torch.stack(per_func_outputs, dim=1)  # (N, num_funcs, rec_num)
    ...
    def _generate_and_score_completions(
        ...
        eos_mask = (sequence_indices == eos_idx.unsqueeze(1)) & completion_mask.bool()
        terminated_with_eos = is_eos.any(dim=1)  # (N_local,)
        ...
        # Calculate rewards for each reward function. rewards_per_func aggregates rewards across all processes.
        ...
        advantages_items_local = advantages_items[process_slice]                # (N_local, rec_num)

        # Build segmentation (unbounded IDs), then broadcast per-item advantages only to the first rec_num items
        item_seg_ids, item_content_mask, overflow_token_mask = self._segment_items_from_tokens(
            completion_ids, completion_mask, max_items=rec_num
        )  # (N_local,T) each

        # How many items were emitted in each sequence? (natural indices)
        with torch.no_grad():
            per_row_max = torch.where(item_seg_ids >= 0, item_seg_ids,
                                      torch.tensor(-1, device=item_seg_ids.device)).max(dim=1).values
            items_emitted = (per_row_max + 1).clamp(min=0)  # 0..(>=rec_num)
            self._metrics[mode]["items/detected"].extend(items_emitted.cpu().tolist())

        # Base broadcast: clamp seg ids into [0, rec_num-1], then mask out overflow positions
        seg_ids_clamped = item_seg_ids.clamp(min=0, max=rec_num - 1)
        advantages_tokens = advantages_items_local.gather(1, seg_ids_clamped)  # (N_local, T)
        advantages_tokens = advantages_tokens * item_content_mask.float()      # zero out overflow
        advantages_tokens = advantages_tokens * (~eos_mask).float() # Consider EOS token seperately

        # === Length shaping (optional) ===
        if self.apply_extra_length_shaping:
            has_overflow = overflow_token_mask.any(dim=1)  # (N_local,)
            exact_len = (items_emitted >= rec_num) & (~has_overflow) & terminated_with_eos
            early_stop = (items_emitted < rec_num) & terminated_with_eos

            # A) Overflow: penalize ALL tokens beyond the 20th item
            advantages_tokens = advantages_tokens + self.extra_token_penalty * overflow_token_mask.float()

            # B) Exactly rec_num items and ended with EOS: reward EOS token
            if exact_len.any():
                row_mask = exact_len.float().unsqueeze(1)  # (N,1)
                advantages_tokens = advantages_tokens + row_mask * (self.end_of_list_reward * eos_mask.float())

            # C) Ended early with EOS: penalize EOS token
            if early_stop.any():
                row_mask = early_stop.float().unsqueeze(1)  # (N,1)
                advantages_tokens = advantages_tokens + row_mask * (self.early_stop_penalty * eos_mask.float())

        # For loss masking, include both item tokens and overflow (so penalties/rewards apply)
        item_token_mask = (item_content_mask | overflow_token_mask).int()
        ...
        # ---- Logging: per-position reward mean/std across groups ----
        mode = "train" if self.model.training else "eval"
        for j in range(rec_num):
            pos_means = rewards_items[:, j].view(Bglob, G).mean(dim=1)
            pos_stds  = rewards_items[:, j].view(Bglob, G).std(dim=1)
            self._metrics[mode][f"rewards/pos_{j+1}/mean"].append(pos_means.nanmean().item())
            self._metrics[mode][f"rewards/pos_{j+1}/std"].append(pos_stds.nanmean().item())

        is_std_zero = torch.isclose(group_stds_items.mean(dim=1), torch.zeros_like(group_stds_items.mean(dim=1)))
        self._metrics[mode]["reward"].append(group_means_items.mean().item())
        ...
        output = {
            ...
            "advantages": advantages_tokens,
            "item_token_mask": item_token_mask,
            "item_seg_ids": item_seg_ids,
        }
    ...
    def _compute_loss(self, model, inputs):
        ...
        # Use item_token_mask if available (excludes newline tokens); otherwise fall back to completion_mask
        mask_for_loss = inputs.get("item_token_mask", completion_mask).int()

        if self.top_entropy_quantile < 1.0:
            entropy_mask = self.get_high_entropy_mask(entropies, mask_for_loss, 1 - self.top_entropy_quantile)
        else:
            entropy_mask = None
        ...
        if self.importance_sampling_level == "token":
            ...
            log_importance_weights = (log_ratio * mask_for_loss).sum(-1) / mask_for_loss.sum(-1).clamp(min=1.0)
            log_importance_weights = log_importance_weights.unsqueeze(-1)
        elif self.importance_sampling_level == "item":
            # Item-level importance sampling: geometric mean over tokens in each *valid* item (0..rec_num-1).
            seg_ids = inputs["item_seg_ids"]  # (B,T), natural IDs
            rec_num = getattr(self, "rec_num", None)
            if rec_num is None:
                raise ValueError("GRPORecTrainer.rec_num must be set for item-level importance sampling.")

            # Prefer item_token_mask (includes overflow)
            mask_for_loss_local = inputs.get("item_token_mask", completion_mask).to(torch.bool)

            device = seg_ids.device
            B, T = seg_ids.size()

            # Add one extra bin for overflow
            Imax = rec_num + 1

            # Clamp seg_ids into [0, rec_num], where:
            #   - seg_ids < 0 → 0 (padding safe)
            #   - seg_ids in [0..rec_num-1] → unchanged (normal items)
            #   - seg_ids >= rec_num → rec_num (overflow bin)
            seg_ids_clamped = seg_ids.clamp(min=0, max=rec_num).long()

            common_dtype = log_ratio.dtype
            sums   = torch.zeros((B, Imax), dtype=common_dtype, device=device)
            counts = torch.zeros((B, Imax), dtype=common_dtype, device=device)

            # Contributions: only include tokens where mask_for_loss_local == True
            src_vals = log_ratio.to(common_dtype) * mask_for_loss_local.to(common_dtype)

            sums.scatter_add_(1, seg_ids_clamped, src_vals)
            counts.scatter_add_(1, seg_ids_clamped, mask_for_loss_local.to(common_dtype))

            means = sums / counts.clamp(min=1.0)  # (B,Imax)
            log_importance_weights = means.gather(1, seg_ids_clamped)  # (B,T)


        else:
            raise ValueError(
                f"Unknown importance sampling level: {self.importance_sampling_level}. "
                "Possible values are 'token', 'sequence', and 'item'."
            )
        ...
        # Broadcast advantages appropriately: (B,) or (B,T)
        adv = advantages.unsqueeze(1) if advantages.ndim == 1 else advantages
        ...
        if self.loss_type == "grpo":
            loss = ((per_token_loss * mask_for_loss).sum(-1) / mask_for_loss.sum(-1).clamp(min=1.0)).mean()
        elif self.loss_type == "bnpo":
            loss = (per_token_loss * mask_for_loss).sum() / mask_for_loss.sum().clamp(min=1.0)
        elif self.loss_type == "dr_grpo":
            loss = (per_token_loss * mask_for_loss).sum() / (per_token_loss.size(0) * self.max_completion_length)
        else:
        ...
        completion_token_count = mask_for_loss.sum().clamp(min=1.0)

        def masked_batch_mean(x):
            if x.shape[1] == 1:  # when importance_sampling_level == "sequence"
                return x.mean()
            else:
                return (x * mask_for_loss).sum() / completion_token_count
        ...
        is_low_clipped = (coef_1 < 1 - self.epsilon_low) & ((adv if adv.ndim == 2 else adv) < 0)
        is_high_clipped = (coef_1 > 1 + self.epsilon_high) & ((adv if adv.ndim == 2 else adv) > 0)
        ...
        self._metrics[mode]["clip_ratio/low_min"].append(gathered_low_clip.nanmin().item() if hasattr(gathered_low_clip, "nanmin") else float(gathered_low_clip.min()))
        gathered_high_clip = self.accelerator.gather(high_clip)
        self._metrics[mode]["clip_ratio/high_mean"].append(gathered_high_clip.nanmean().item())
        self._metrics[mode]["clip_ratio/high_max"].append(gathered_high_clip.nanmax().item() if hasattr(gathered_high_clip, "nanmax") else float(gathered_high_clip.max()))
    ...
    def log(self, logs: dict[str, float], start_time: Optional[float] = None) -> None:
        ...
        for key, val in self._metrics[mode].items():
            if key == "items/detected":
                if len(val) > 0:
                    metrics["items/detected_mean"] = float(sum(val) / len(val))
                    metrics["items/detected_min"]  = float(min(val))
                    metrics["items/detected_max"]  = float(max(val))
                else:
                    metrics["items/detected_mean"] = metrics["items/detected_min"] = metrics["items/detected_max"] = float("nan")
            else:
                metrics[key] = float(sum(val) / max(len(val), 1))
        # average the metrics
